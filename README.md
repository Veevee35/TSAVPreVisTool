...
Panel definitions store real-world cabinet dimensions and native pixel resolution. The wall links that cabinet into rows and columns, calculates total screen resolution, generates serpentine cabinet topology records, and maps the resulting screen to an exact X/Y rectangle on a 4096×2160 processor canvas. Each column can be curved from -15° to +15° in 0.5° steps, and an interactive panel grid supports square, diagonal, and rounded corner cabinets in all four corner directions. The editor tool also includes a live canvas preview, overflow validation, an NDI/Media Source picker, Rectangle RGB and Round RGB physical subpixel simulations, and buttons to create a new wall or update the selected one.
