...
Panel definitions store real-world cabinet dimensions and native pixel resolution. The wall links that cabinet into rows and columns, calculates total screen resolution, generates serpentine cabinet topology records, and maps the resulting screen to an exact X/Y rectangle on a 4096×2160 processor canvas. The editor tool includes a live canvas preview, overflow validation, an NDI/Media Source picker, Rectangle RGB and Round RGB physical subpixel simulations, and buttons to create a new wall or update the selected one.
