...
$TsavExpected = @('TSAV.LightingConsole.PatchPlan', 'TSAV.LightingConsole.NativeFixtures', 'TSAV.LightingConsole.ShowPlayback', 'TSAV.LightingConsole.ProjectRoundTrip', 'TSAV.LightingConsole.MatrixFixtures', 'TSAV.NativeScene.Generators', 'TSAV.NativeLaser.ILDA', 'TSAV.NativeRig.Exchange', 'TSAV.LightingConsole.Executors', 'TSAV.NativeScene.RenderReview')
$TsavArgs = @(
    ('"' + $TsavProject + '"'), '/Engine/Maps/Entry',
    '-DisablePlugins=SuperStage', '-Unattended', '-NoSound', '-NoSplash',
...
if ($Render) { $TsavArgs += '-TSAVNativeVisuals' } else { $TsavArgs += '-NullRHI' }
