...
3. Enter Columns and Rows. Set each seam between neighboring columns from -15° to +15° in 0.5° steps; repeated bends form a connected arc. Choose **Square**, **Empty**, one of four full corner-to-corner diagonal halves, or one of four rounded corners, then click panels in the shape grid to paint that state; right-click restores a square panel. Empty cells generate no cabinet geometry or signal link, and the outer trim follows the remaining wall silhouette. Media UVs retain their original full-grid positions so irregular walls remain predictable on the processor canvas. The tool also calculates the active cabinet count, wall size, total grid resolution, and pixel pitch. Choose **Off**, **Rectangle RGB**, or **Round RGB** under Subpixel Layout. Strength blends from solid video at 0% to the full emitter mask at 100%.
