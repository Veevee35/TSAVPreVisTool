...
3. Enter Columns and Rows. Set each vertical column seam and horizontal row seam from -90° to +90° in 0.5° steps; repeated bends form connected two-axis curves. Row bends keep the first row facing forward, so a 90° seam can fold a flat enabled run underneath the main wall as a bottom screen, including irregular layouts with surrounding empty cells. Under **Internal panel curves**, each column has its own enable toggle and independent signed circular arc sweeps for the left and right halves of every panel. Each sweep ranges from -90° to +90° in 0.5° steps. The left half changes the outgoing wall direction, the right half continues from it, and every following column begins at the resulting connected edge and heading; positive and negative values curve in opposite directions and can form S-curves. Editing either value automatically enables that column, and **Apply curves to selected wall** updates it immediately. Existing radius-based walls migrate to equivalent angles automatically. Use **Flat row overrides** to turn consecutive selected rows into a centered planar top or bottom surface while retaining their row-seam orientation and excluding internal column arcs. The surface uses only the enabled footprint, so empty columns do not stretch it or deform the curved wall. Every bend uses a connected video-plane grid, keeping the front image watertight while **Rear Cabinet Gap** reserves clearance only behind it. Every cabinet side, plus exposed outer trim, retreats inward and backward at 45° so rear bodies clear adjacent panels during tight folds. Choose **Square**, **Empty**, one of four full corner-to-corner diagonal halves, or one of four corner-spanning rounded edges. Click a single panel to paint it, or drag a rectangular selection and use **Apply to selection** to update the whole region; **Clear selection** removes the highlight and right-click restores an individual square panel. **Rounded edge radius** accepts any value from 0.5 m upward with up to ten decimal places. Empty cells generate no cabinet geometry or signal link, and the chamfered outer trim follows the remaining wall silhouette. Media UVs retain their original full-grid positions so irregular walls remain predictable on the processor canvas. The tool also calculates the active cabinet count, wall size, total grid resolution, and pixel pitch. Choose **Off**, **Rectangle RGB**, **Round RGB**, or **Round Linear** under Subpixel Layout. Strength blends from solid video at 0% to the full emitter mask at 100%.
...
The bundled display material repeats the selected RGB emitter layout once per native wall pixel, after the incoming canvas has been cropped and sampled at native resolution. **Rectangle RGB** uses vertical red/green/blue stripe emitters; **Round RGB** uses a triangular cluster of round emitters; **Round Linear** uses the supplied vertical red/green/blue round-emitter layout. At a distance, texture filtering blends them back into the image; close up, the individual emitters and black mask are visible. **Off** skips the mask for a conventional solid-video preview.
