...
3. Enter Columns and Rows. Set each vertical column seam and horizontal row seam from -90° to +90° in 0.5° steps; repeated bends form connected two-axis curves. Every bend uses a shared video-plane corner grid, keeping the front image watertight while **Rear Cabinet Gap** reserves clearance only behind it. Exposed top, bottom, left, and right trim retreats behind the image at 45° instead of projecting through tightly folded panels. Choose **Square**, **Empty**, one of four full corner-to-corner diagonal halves, or one of four rounded corners, then click panels in the shape grid to paint that state; right-click restores a square panel. Empty cells generate no cabinet geometry or signal link, and the chamfered outer trim follows the remaining wall silhouette. Media UVs retain their original full-grid positions so irregular walls remain predictable on the processor canvas. The tool also calculates the active cabinet count, wall size, total grid resolution, and pixel pitch. Choose **Off**, **Rectangle RGB**, or **Round RGB** under Subpixel Layout. Strength blends from solid video at 0% to the full emitter mask at 100%.
